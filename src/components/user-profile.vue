<template>
  <v-card variant="tonal" color="info">
    ...
    <v-card-actions v-if="this.actions">
      <v-spacer></v-spacer>
      <v-btn v-if="this.$store.state.loginKey" size="large" variant="tonal" color="info" text @click="logout">
        Log out
        <v-icon right>mdi-logout</v-icon>
      </v-btn>
      <v-btn v-if="!this.$store.state.loginKey" size="large" variant="tonal" color="info" :to="'/login'">
        Log in
