<template>
  <v-container>
    ...
    <h2 class="text-h5 text-decoration-underline my-3">
      Edit library locations
    </h2>
            ...
            <v-toolbar flat color="grey-lighten-5">
              ...
              <v-dialog persistent overlay-opacity="0.1" content-class="elevation-0" v-model="dialogDelete"
                ...
                <v-card>
                  ...
                  <v-card-text>
                    This action will remove the library. If the library has
                    closed it should not be removed. The closed year can be
                    entered in the library details to mark it as closed and
                    retain the details.
                  </v-card-text>
                  <v-card-actions>
                    <v-spacer />
...
<script>
import MarkDownData from '../markdown/librariesdataedit.md?raw'
...
export default {
  ...
  methods: {
    ...
    loadLibraries: async function (service) {
      ...
      const self = this
      const libraries = await schemaHelper.getSchemaData(
        'libraries',
        service.code
      )
    ...
    publishChanges: async function () {
      const librariesToPublish = JSON.parse(JSON.stringify(this.libraries))
