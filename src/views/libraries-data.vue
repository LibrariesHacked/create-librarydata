  ...
  <v-container>
    ...
    <v-sheet color="grey-lighten-5" rounded elevation="0" class="px-5 py-5">
      ...
      <v-alert class="my-2" icon="mdi-numeric-2-circle" text="To edit library data, select the edit icon for each library.
        Changes are not saved until you select publish changes."></v-alert>

      <v-card elevation="0" color="grey-lighten-5">
        <v-card-text>
          <v-data-table hover items-per-page="5" no-data-text="No libraries loaded" :headers="headers" :items="libraries"
            class="elevation-0">
            <template #top>
              <v-toolbar flat>
                <v-spacer></v-spacer>
                <v-dialog persistent v-model="dialogMainLibrary" max-width="500px" overlay-opacity="0.3"
                  ...
                  <template v-slot:activator="{ on, attrs }">
                    <v-btn v-bind="attrs" variant="tonal" color="success" v-on="on">
      ...
      <v-btn size="large" variant="tonal" color="info" class="ma-2" append-icon="mdi-content-save" v-on:click="download">
        Save file
      </v-btn>

      <v-btn size="large" variant="tonal" color="success" append-icon="mdi-cloud-upload" v-on:click="publishChanges">
        Publish changes
      </v-btn>
